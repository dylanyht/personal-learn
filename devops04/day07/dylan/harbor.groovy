...
def DeleteArtifactTag(projectName,repoName, tagName){
    harborAPI = "https://harbor.bltest.kutesmart.cn/api/v2.0/projects/${projectName}/repositories/${repoName}"
    apiURL = "artifacts/${tagName}/tags/${tagName}"
    sh """ curl -X DELETE "${harborAPI}/${apiURL}" -H "accept: application/json"  -u admin:Admin000 """
}
...
def GetArtifactTag(projectName,repoName ){
    ...
    response = sh returnStdout: true, script:  """curl -X GET "${harborAPI}/${apiURL}" -H "accept: application/json" -u admin:Admin000 """
